...
# Check if we're in lightweight/cloud mode (no heavy ML models)
...
if not LIGHTWEIGHT_MODE:
    ...
    HEAVY_MODELS_AVAILABLE = False
    print("[CLOUD MODE] Heavy NLP models disabled. Using Gemini AI + keyword matching.")
...
class NLPEngine:
    ...
    def load_models(self):
        """Loads models. Skips heavy ones in LIGHTWEIGHT_MODE."""
        if LIGHTWEIGHT_MODE or not HEAVY_MODELS_AVAILABLE:
            print("[INFO] Running in lightweight mode — NLP models skipped.")
            self.is_ready = False
            return
    ...
    def predict_intent(self, text):
        """Returns (intent, confidence, debug_info). Falls back to keyword matching if models not loaded."""
        if not self.is_ready or len(self.kb_embeddings) == 0:
            ...
            for item in self.faqs:
                ...
                if item.get('question_hi') and (item['question_hi'] in text or text in item['question_hi']):
                    return item['intent'], 1.0, [("keyword_match_hi", 1.0)]
                if item.get('question_ta') and (item['question_ta'] in text or text in item['question_ta']):
    ...
    def get_gemini_response(self, api_key, query, context=None, lang='en'):
        ...
        if not genai:
            return "Error: google-generativeai package not installed.", "Error", "error"
        ...
        try:
            ...
            6. If you do not know the answer, do not guess. Simply reply with 'I need more info'.
            """
    ...
    def extract_entities(self, text):
        """Extracts entities using regex (spaCy optional)."""
        entities = {}
